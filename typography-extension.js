(() => {
  ...
  const promptFont = (font) => `השתמש בגופן '${font.name}' באתר החדש. שימוש מומלץ: ${font.use}. התחושה העיצובית: ${font.tone}. שמור על עברית RTL מלאה, התאמה לנייד ולמחשב, קריאות גבוהה, ריווח שורות נוח, ניגודיות טובה, ותוכן אמיתי בלבד. הפרד בין HTML, CSS ו-JavaScript. אם אתה משתמש ב-Google Fonts, טען את הגופן בצורה תקינה והוסף fallback של Arial/sans-serif.`;
  const promptSize = (size) => `השתמש בגודל הטקסט '${size.name}' באתר החדש. הערך המדויק הוא ${size.value} (${size.px}). שימוש מומלץ: ${size.use}. שמור על טיפוגרפיה עברית RTL מלאה, התאמה לנייד ולמחשב, קריאות גבוהה, היררכיית כותרות ברורה, ותוכן אמיתי בלבד. הפרד בין HTML, CSS ו-JavaScript.`;
  const promptPair = (pair) => `השתמש במערכת הטיפוגרפיה '${pair.name}' באתר החדש. גופן ראשי: ${pair.font}. גודל כותרת: ${pair.heading}. גודל גוף: ${pair.body}. שימוש מומלץ: ${pair.use}. בנה היררכיית טקסט מלאה: Hero, כותרות אזור, כרטיסיות, טבלאות, כפתורים וטקסט גוף. שמור על RTL מלא, התאמה לנייד, קריאות גבוהה, ללא גלילה אופקית, תוכן אמיתי בלבד, והפרדה בין HTML, CSS ו-JavaScript.`;
  ...
  function cardFont(font) {
    return `<article class="type-card" id="${font.id}"><span class="type-badge">גופן</span><h3>${font.name}</h3><div class="type-sample" style="font-family:'${font.cssName}',Arial,sans-serif"><strong>תצוגת כותרת בעברית</strong><br>תצוגת טקסט אתר קריא, נקי ומותאם לנייד.</div><div class="type-meta">${font.use}<br>${font.tone}</div><div class="type-value">font-family: '${font.cssName}', Arial, sans-serif;</div><div class="type-actions"><button class="copy-main" data-copy-font-prompt="${font.id}">העתק Prompt לגופן</button><button data-copy-font-css="${font.id}">העתק CSS</button><button data-copy-font-name="${font.id}">העתק שם גופן</button></div></article>`;
  }

  function cardSize(size) {
    return `<article class="type-card" id="${size.id}"><span class="type-badge">גודל</span><h3>${size.name}</h3><div class="type-sample" style="font-size:${size.value}">תצוגת טקסט בגודל הזה</div><div class="type-meta">${size.use}</div><div class="type-value">font-size: ${size.value}; /* ${size.px} */</div><div class="type-actions"><button class="copy-main" data-copy-size-prompt="${size.id}">העתק Prompt לגודל</button><button data-copy-size-css="${size.id}">העתק CSS</button><button data-copy-size-value="${size.id}">העתק ערך</button></div></article>`;
  }

  function cardPair(pair) {
    return `<article class="type-card" id="${pair.id}"><span class="type-badge">שילוב</span><h3>${pair.name}</h3><div class="type-sample" style="font-family:'${pair.font}',Arial,sans-serif"><strong style="font-size:${pair.heading}">כותרת</strong><br><span style="font-size:${pair.body}">תצוגת טקסט גוף באתר אמיתי.</span></div><div class="type-meta">${pair.use}</div><div class="type-value">font: ${pair.font}; heading: ${pair.heading}; body: ${pair.body};</div><div class="type-actions"><button class="copy-main" data-copy-pair-prompt="${pair.id}">העתק Prompt לשילוב</button><button data-copy-pair-css="${pair.id}">העתק CSS</button></div></article>`;
  }
