...
## 2026-07-15 — תוכנית קלוטקורד גרסה 2
- המחקר צומצם לריפו הליבה הגדולים והמשמעותיים בלבד.
- נותחו דפוסי commits, PRs, CI, state, permissions, agents, scripts ואוטומציות.
- הוגדר זיכרון דרישות אישי ומתמשך כיעד המרכזי.
- נוספו `research/MEMORY_SYSTEM_REQUIREMENTS.md` ו־`research/CLAUDE_CODE_WORK_PATTERNS.md`.
- תוכנית העבודה המרכזית הוחלפה בגרסה 2 שמרכזת את כל דרישות יניב ואת המלצות המחקר.
- נוספו דרישות מפורשות לחשיבה ביקורתית, אי־חזרה על שאלות, עצמאות עם שערי בטיחות, טיפול בתיקון כושל, evidence ואיסור על אוטומציה שמשכתבת אמת.
- ה־Skill הראשון שנבחר לפיילוט הוא `capture-requirement`.
- סדר היישום עודכן: inventory → baseline → פיילוט זיכרון → `capture-requirement` → בטיחות → LSP → `ui-verify` → הרחבה.
- `research/IMPLEMENTATION_PRIORITY.md` נמחק משום ששכפל את ה־Roadmap ואת תוכנית גרסה 2.
- README, CLAUDE, CURRENT_STATE, PROJECT_GOALS, ROADMAP, DECISIONS והמדריך היומי סונכרנו למבנה החדש.
